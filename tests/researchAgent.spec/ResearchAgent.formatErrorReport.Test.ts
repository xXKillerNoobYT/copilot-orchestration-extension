...
`);
        ...
        /** @aiContributed-2026-02-04 */
        it('should handle long query and errorMessage gracefully', () => {
            const query = 'A'.repeat(1000);
            const errorMessage = 'B'.repeat(1000);
            const mockDate = new Date('2023-01-01T12:00:00Z');
            jest.spyOn(global, 'Date').mockImplementation(() => mockDate as unknown as string);

            const result = researchAgent['formatErrorReport'](query, errorMessage);

            expect(result).toContain(`**Query:** ${'A'.repeat(1000)}`);
            expect(result).toContain(`Failed to generate research report: ${'B'.repeat(1000)}`);
        });
