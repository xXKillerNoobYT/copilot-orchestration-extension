...
describe('ConversationsTreeDataProvider', () => {
    ...
    describe('formatRelativeTime', () => {
        ...
        it('should return "Unknown time" for null timestamp', () => {
            const result = provider.formatRelativeTime(null as unknown as number);
            expect(result).toBe('Unknown time');
        });

        /** @aiContributed-2026-02-03 */
        it('should return "Unknown time" for undefined timestamp', () => {
            const result = provider.formatRelativeTime(undefined as unknown as number);
            expect(result).toBe('Unknown time');
        });
        ...
        it('should return "just now" for timestamps within the last 60 seconds', () => {
            ...
            const result = provider.formatRelativeTime(now - 30 * 1000);
        ...
        it('should return "1 minute ago" for timestamps 1 minute ago', () => {
            const now = Date.now();
            jest.spyOn(Date, 'now').mockReturnValue(now);
            const result = provider.formatRelativeTime(now - 60 * 1000);
        ...
        it('should return "5 minutes ago" for timestamps 5 minutes ago', () => {
            const now = Date.now();
            jest.spyOn(Date, 'now').mockReturnValue(now);
            const result = provider.formatRelativeTime(now - 5 * 60 * 1000);
        ...
        it('should return "1 hour ago" for timestamps 1 hour ago', () => {
            const now = Date.now();
            jest.spyOn(Date, 'now').mockReturnValue(now);
            const result = provider.formatRelativeTime(now - 60 * 60 * 1000);
        ...
        it('should return "3 hours ago" for timestamps 3 hours ago', () => {
            const now = Date.now();
            jest.spyOn(Date, 'now').mockReturnValue(now);
            const result = provider.formatRelativeTime(now - 3 * 60 * 60 * 1000);
        ...
        it('should return "1 day ago" for timestamps 1 day ago', () => {
            const now = Date.now();
            jest.spyOn(Date, 'now').mockReturnValue(now);
            const result = provider.formatRelativeTime(now - 24 * 60 * 60 * 1000);
        ...
        it('should return "7 days ago" for timestamps 7 days ago', () => {
            const now = Date.now();
            jest.spyOn(Date, 'now').mockReturnValue(now);
            const result = provider.formatRelativeTime(now - 7 * 24 * 60 * 60 * 1000);
        ...
        it('should handle future timestamps gracefully', () => {
            const now = Date.now();
            jest.spyOn(Date, 'now').mockReturnValue(now);
            const result = provider.formatRelativeTime(now + 1000);
            expect(result).toBe('just now');
        });
    });
});