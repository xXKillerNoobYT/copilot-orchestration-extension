...
You have generated unit test code with <<<TestFramework>>> test framework. The code was linted with eslint but failed.

**Correct the test code** based on the eslint error data below. Resolve every error—do not skip any.
...
Return **ONLY** the corrected test code in a single markdown code block with triple backticks and no explanations:
```ts
[corrected test code]
```