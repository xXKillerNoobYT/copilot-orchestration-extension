# I have a class `<<<ClassName>>>`'s member `<<<FunctionName>>>` in the file <<<SourceCode>>>, and now it changed. I need to upgrade the unit test file <<<SpecFileName>>>.
# Guidelines:
...
title: Repair Class Member Unit Tests
---
...
The class `<<<ClassName>>>`'s member function `<<<FunctionName>>>` in file `<<<SourceFileName>>>` has changed. Update the unit test file `<<<SpecFileName>>>` to match the new implementation.
...
- If the function is private, use `as any` to access it.
...
- Do not redefine or overwrite the class/methods under test—only spy or mock dependencies.
...
**Updated function `<<<FunctionName>>>` implementation:**
```<<<Language>>>
<<<SourceCode>>>
```
