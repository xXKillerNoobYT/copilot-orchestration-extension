...
title: Generate Class Member Unit Tests
---
...
Write comprehensive unit tests with <<<TestFramework>>> for the class `<<<ClassName>>>`'s member function `<<<FunctionName>>>` in a new spec file (`<<<SpecFileName>>>`).
