...
title: Fix Test Data Errors (Step 2)
---
...
The test code failed when run. **Replace the incorrect expected values** with the actual correct data.
...
Return **ONLY** the corrected test code in a code block with no explanations:
```ts
[corrected test code]
```